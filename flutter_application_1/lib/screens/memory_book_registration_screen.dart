...
class MemoryBookRegistrationData {
  ...
  /// Сериализация для отправки на бэкенд через AuthService.saveProfile.
  /// Названия полей (name_mode) — формат, который ждёт сервер
  /// (см. backend/app/schemas.py: ProfileIn).
...
class _MemoryBookRegistrationScreenState
  ...
  /// Открывает диалог с Люми ещё раз — в режиме просмотра, сразу на
  /// последней реплике («Наше путешествие начинается»). Прогресс при
  /// этом не трогается: игрок просто смотрит уже пройденную сцену и
  /// возвращается обратно на эту же страницу «Книги воспоминаний».
  /// Со стрелочкой "назад" внутри самой сцены можно долистать назад к
  /// более ранним репликам, если захочется пересмотреть весь диалог.
  void _openLumiDialogueReview() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (ctx) => AwakeningSceneScreen(
          readOnly: true,
          initialBeatIndex: awakeningScript.length - 1,
          initialPageIndex: 0,
    ...
    return Scaffold(
      body: Stack(
        ...
        children: [
          ...
          // Стрелочка "назад к разговору с Люми" — видна только на самой
          // первой странице (интро), пока игрок ещё не начал заполнять
          // историю. Открывает диалог заново в режиме просмотра, не
          // трогая сохранённый прогресс.
          if (_step == 0)
            SafeArea(
              child: Align(
