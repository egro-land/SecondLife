...
class BookOpeningVideoScreen extends StatefulWidget {
  ...
  /// Если задан — пролог (AwakeningSceneScreen) откроется сразу на этом
  /// такте вместо самого начала. Используется, когда игрок в прошлый
  /// раз не досмотрел пролог до конца.
  final int? initialBeatIndex;

  /// Страница внутри initialBeatIndex, на которой нужно открыть сцену.
  final int initialPageIndex;

  /// Если true — пролог пропускается полностью, и сразу после видео
  /// открывается регистрация персонажа. Используется, когда игрок уже
  /// досмотрел пролог, но не закончил регистрацию в прошлый раз.
...
class _BookOpeningVideoScreenState extends State<BookOpeningVideoScreen> {
  ...
  void _goToPrologue() {
    ...
    // Пролог уже пройден в прошлый раз, осталась только незаконченная
    // регистрация персонажа — сразу открываем её, не проигрывая пролог
    // заново.
    if (widget.startAtRegistration) {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (ctx) => MemoryBookRegistrationScreen(
            onComplete: (data) => _completeRegistration(ctx, data),
          ),
        ),
      );
      return;
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        ...
        builder: (ctx) => AwakeningSceneScreen(
          ...
          // сразу на нужном такте/странице вместо самого начала.
          initialBeatIndex: widget.initialBeatIndex,
  ...
  /// Сохраняет профиль персонажа и переводит прогресс в "главное меню"
  /// — чтобы при следующем запуске приложение сразу показывало главное
  /// меню (см. AuthGate), а не начинало всё заново.
